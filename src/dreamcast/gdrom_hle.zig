//! GDROM HLE - Used by the BIOS syscalls

pub const GDROMCommand = enum(u32) {
    None = 0,
    PIORead = 16,
    DMARead = 17,
    GetTOC = 18,
    GetTOC2 = 19,
    Play = 20,
    Play2 = 21,
    Pause = 22,
    Release = 23,
    Init = 24,
    Seek = 27,
    Read = 28,
    ReqMode = 30,
    SetMode = 31,
    Stop = 33,
    GetSCD = 34,
    GetSession = 35,
    GetVersion = 40,
    _,
};

command: GDROMCommand = .None,
params: [4]u32 = @splat(0),
result: [4]u32 = @splat(0),

_next_command_id: u32 = 1,
_current_command_id: u32 = 0,

pub fn reset(self: *@This()) void {
    self.command = @enumFromInt(0);
    @memset(&self.params, 0);
    @memset(&self.result, 0);
    self._next_command_id = 1;
    self._current_command_id = 0;
}

pub fn send_command(dc: *Dreamcast, command_code: u32, params: [4]u32) u32 {
    if (dc.gdrom.state != .Standby) return 0;

    dc.gdrom_hle._current_command_id = dc.gdrom_hle._next_command_id;
    dc.gdrom_hle._next_command_id +%= 1;
    if (dc.gdrom_hle._next_command_id == 0) dc.gdrom_hle._next_command_id = 1;

    dc.gdrom.state = .Busy;
    dc.gdrom_hle.command = @enumFromInt(command_code);
    dc.gdrom_hle.params = params;
    @memset(&dc.gdrom_hle.result, 0);

    return dc.gdrom_hle._current_command_id;
}

pub fn mainloop(dc: *Dreamcast) void {
    if (dc.gdrom.state != .Busy) {
        log.debug("  GDROM Mainloop - No command queued", .{});
        return;
    }

    log.info("  GDROM Mainloop - {}", .{dc.gdrom_hle.command});
    for (0..dc.gdrom_hle.params.len) |i|
        log.debug("    [{d}] {X:0>8}", .{ i, dc.gdrom_hle.params[i] });

    switch (dc.gdrom_hle.command) {
        .DMARead, .PIORead => {
            const lba = dc.gdrom_hle.params[0];
            const size = dc.gdrom_hle.params[1];
            const dest = dc.gdrom_hle.params[2] & 0x1FFFFFFF;
            log.info("    GDROM {t} sector={d} size={d} destination=0x{X:0>8}", .{ dc.gdrom_hle.command, lba, size, dest });
            const read = dc.gdrom.disc.?.load_sectors(lba, size, @as([*]u8, @ptrCast(dc._get_memory(dest)))[0 .. 2352 * size]);
            dc.gdrom_hle.result = .{ 0, 0, read, 0 };
            dc.gdrom.state = .Standby;
        },
        .Init => {
            log.warn("    GDROM Init: Unimplemented", .{});
            dc.gdrom.state = .Standby;
        },
        .GetVersion => {
            const dest = dc.gdrom_hle.params[0];
            const version = "GDC Version 1.10 1999-03-31";
            for (0..version.len) |i|
                dc.cpu.write_physical(u8, @intCast(dest + i), version[i]);
            dc.cpu.write_physical(u8, @intCast(dest + version.len), 0x2);
            dc.gdrom.state = .Standby;
        },
        .ReqMode => {
            const dest = dc.gdrom_hle.params[0];
            log.info("    GDROM ReqMode  dest=0x{X:0>8}", .{dest});
            dc.cpu.write_physical(u32, dest + 0, 0); // Speed
            dc.cpu.write_physical(u32, dest + 4, 0x00B4); // Standby
            dc.cpu.write_physical(u32, dest + 8, 0x19); // Read Flags
            dc.cpu.write_physical(u32, dest + 12, 0x08); // Read retry
            dc.gdrom_hle.result = .{ 0, 0, 0xA, 0 };
            dc.gdrom.state = .Standby;
        },
        .SetMode => {
            log.warn("    GDROM SetMode: Unimplemented", .{});
            dc.gdrom.state = .Standby;
        },
        .GetTOC2 => {
            const area = dc.gdrom_hle.params[0];
            const dest = dc.gdrom_hle.params[1];
            log.info("    GDROM GetTOC2: area={d} dest=0x{X:0>8}", .{ area, dest });

            if (dc.gdrom.disc) |disc| {
                const dest_slice = @as([*]u8, @ptrCast(dc._get_memory(dest & 0x1FFFFFFF)))[0..408];
                const bytes_written = disc.write_toc(dest_slice, if (area == 1) .DoubleDensity else .SingleDensity);
                const dest_u32 = std.mem.bytesAsSlice(u32, dest_slice[0..bytes_written]);
                for (dest_u32) |*v|
                    v.* = @byteSwap(v.*);
                dc.gdrom_hle.result = .{ 0, 0, bytes_written, 0 };
            } else {
                dc.gdrom_hle.result = .{ 0xFFFFFFFF, 0, 0, 0 };
            }
            dc.gdrom.state = .Standby;
        },
        .GetSCD => {
            log.warn("    GDROM GetSCD: Unimplemented", .{});
            const len = dc.gdrom_hle.params[1];
            const dest = dc.gdrom_hle.params[2];
            for (0..len) |i|
                dc.cpu.write_physical(u32, @intCast(dest + 4 * i), 0);
            dc.gdrom.state = .Standby;
        },
        else => {
            log.warn("Unhandled GDROM command {}", .{dc.gdrom_hle.command});
            dc.gdrom.state = .Standby;
        },
    }
}

pub fn check_command(dc: *Dreamcast, cmd_id: u32) enum(u32) {
    /// No such request active
    NoRequest = 0,
    /// Request is still being processed
    InProgress = 1,
    /// Request has completed (if queried again, you will get a 0)
    Completed = 2,
    /// Request was aborted
    Aborted = 3,
    /// Request has failed (examine extended status information for cause of failure)
    Failed = 0xFFFFFFFF,
} {
    if (cmd_id != dc.gdrom_hle._current_command_id) {
        dc.gdrom_hle.result = .{ 5, 0, 0, 0 };
        return .NoRequest;
    }
    if (dc.gdrom.state != .Standby) return .InProgress;
    dc.gdrom_hle._current_command_id = 0;
    return .Completed;
}

pub fn serialize(self: *@This(), writer: *std.Io.Writer) !usize {
    var bytes: usize = 0;
    bytes += try writer.write(std.mem.asBytes(&self.command));
    bytes += try writer.write(std.mem.sliceAsBytes(self.params));
    bytes += try writer.write(std.mem.sliceAsBytes(self.result));
    bytes += try writer.write(std.mem.asBytes(&self._next_command_id));
    bytes += try writer.write(std.mem.asBytes(&self._current_command_id));
    return bytes;
}

pub fn deserialize(self: *@This(), reader: *std.Io.Reader) !void {
    try reader.readSliceAll(std.mem.asBytes(&self.command));
    try reader.readSliceAll(std.mem.sliceAsBytes(self.params));
    try reader.readSliceAll(std.mem.sliceAsBytes(self.result));
    try reader.readSliceAll(std.mem.asBytes(&self._next_command_id));
    try reader.readSliceAll(std.mem.asBytes(&self._current_command_id));
}

const std = @import("std");
const Dreamcast = @import("dreamcast.zig").Dreamcast;

const log = std.log.scoped(.gdrom_hle);
