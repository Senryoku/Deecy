...
pub const MaplePort = union(enum) {
    none: void,
...
pub const MapleHost = struct {
    ...
    pub fn transfer(self: *MapleHost, dc: *Dreamcast, data: [*]u32) void {
        ...
        while (idx < 1024) {
            ...
            switch (instr.pattern) {
                .Normal => {
                    transferred_words += self.ports[instr.port_select].handle_command(dc, data[idx .. idx + instr.transfer_length + 2]);
    ...
    pub fn flush_vmus(self: *@This()) void {
        ...
        for (&self.ports) |*port| {
            switch (port.*) {
                .emulated => |*e| {
                    for (&e.subperipherals) |*maybe_peripheral| {
