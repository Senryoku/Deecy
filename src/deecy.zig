...
const Configuration = struct {
    ...
    frame_limiter: enum { Off, Auto, @"120Hz", @"100Hz", @"60Hz", @"59.94Hz", @"50Hz" } = .Auto,
...
pub fn create(allocator: std.mem.Allocator, io: std.Io, flags: packed struct { wayland: bool }) !*@This() {
...

    {
...

        {
            ...
            if (self.config.fullscreen) fallback: {
                ...
                const mode = monitor.getVideoMode() catch |err| {
                    deecy_log.err("Failed to get video mode: {t}", .{err});
                ...
                self.previous_window_position = .{ .x = 0, .y = 0, .w = initial_width, .h = initial_height };
            }
            ...
            if (!flags.wayland and !self.config.fullscreen)
    ...
    for (self.config.controllers, 0..) |c, idx| {
        if (c.enabled)
            try self.enable_port(@intCast(idx), true);
    }
...
pub fn load_vmu(self: *@This(), port: u8, slot: u8, vmu_path: []const u8) !void {
    ...
    switch (self.dc.maple.ports[port]) {
        .emulated => |*e| {
            ...
            e.subperipherals[slot].?.VMU.on_timer_alarm = .{ .function = @ptrCast(&vmu_alarm_callback), .userdata = self };
        },
        else => {},
    }
}
...
pub fn enable_port(self: *@This(), port: u8, value: bool) !void {
    ...
    if (value) {
        ...
        if (self.dc.maple.ports[port] == .emulated) {
...
pub fn get_dc_keyboard(self: *@This()) ?*DreamcastModule.Maple.Keyboard {
    for (&self.dc.maple.ports) |*port| {
        switch (port.*) {
            .emulated => |*e| {
                if (e.main == .Keyboard) return &e.main.Keyboard;
            },
            else => {},
        }
    }
...
pub fn get_dc_mouse(self: *@This()) ?*DreamcastModule.Maple.Mouse {
    for (&self.dc.maple.ports) |*port| {
        switch (port.*) {
            .emulated => |*e| {
                if (e.main == .Mouse) return &e.main.Mouse;
            },
            else => {},
        }
    }
...
fn rewind_update_preview(self: *@This()) void {
    const texture = if (self.rewind.selected_snapshot < self.rewind.snapshots.items.len)
        self.rewind.snapshots.items[@intCast(self.rewind.selected_snapshot)].preview.texture
    else
        self.rewind.current_frame.texture;
    if (texture.id != 0) {
        // Fullscreen preview. Re-uses the renderer framebuffer for simplicity.
        Rewind.copy_texture(self.gctx, texture, self.renderer.resized_framebuffer.texture, self.renderer.resolution);
    }
}
