...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    let heads_index = global_id.y * oit_tmv_uniforms.target_width + global_id.x;

    let frag_count = min(MaxVolumeFragments, modvol_heads.fragment_count[heads_index]);
    ...
    for(var i = 0u; i < frag_count; i++) {
        let index = i * oit_tmv_uniforms.pixels_per_slice + heads_index; // See the comment in the fragment shader
